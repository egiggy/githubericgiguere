jQuery(document).ready(function() {
	...
	// titleParth =  jQuery(this).parents(".slider-content").prev().children(".slider-item-title-text");
	 ...
	 imgurl = jQuery('img',html).attr('src');
	 imgurl_temp = imgurl;
	 var i = imgurl.indexOf('wp-content');
	 imgurl = imgurl.substr(i);
	  		
	 	
	 thumbParth.html("<div style='width: 65px; height: 50px; border: 1px #ddd solid; margin-bottom: 10px;'><img src="+imgurl_temp+" height=50 width=65></div>");
		//titleParth.html(imgurl);
		formfield.val(imgurl);
	 tb_remove();
	 }
