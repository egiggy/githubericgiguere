...
function button_func($atts, $content) {
	...
	$return_html = '<input type="button" class="button '.$size.' '.$align.'" style="text-shadow:0 0 0 #fff;font-weight:normal;background: -webkit-gradient(linear, left top, left bottom, from('.$bg_color_light.'), to('.$bg_color.'));background: -moz-linear-gradient(top,  '.$bg_color_light.',  '.$bg_color.');filter:  progid:DXImageTransform.Microsoft.gradient(startColorstr=\''.$bg_color_light.'\', endColorstr=\''.$bg_color.'\');border:1px solid '.$border_color.';color:'.$text_color.';'.$style.'" value="'.$content.'"';
...
function home_portfolios_func($atts, $content) {
	...
	$return_html = '<div class="one_fourth"><strong class="header">'.$title.'</strong><p>'.$content.'</p></div>';
	...
	$return_html = '<div class="post_header"><h3><a href="javascript:;">'.$title.'</a></h3></div>';
	...
	foreach($recent_portfolios_arr as $key => $recent_portfolio)
	{
		...
		{	
		    ...
		    $current_float = 'float:left;width:230px;height:140px;margin-right:20px;margin-bottom:18px;';
		}
		...
		$return_html.= '<div title="home_portfolios" class="'.$column_class.'">';
		
		switch($portfolio_type)
		{
			case 'External Link':
			default:
				$return_html.= '<div style="'.$current_float.'">';
				$return_html.= '<a title="'.$recent_portfolio->post_title.'" href="'.$permalink_url.'" style="display:block;">';
				$return_html.= '<div class="overlay_detail" style="width:238px">
									<div>
				...
				';
				$return_html.= '<img src="'.get_stylesheet_directory_uri().'/timthumb.php?src='.$image_url[0].'&amp;h=140&amp;w=235&amp;zc=1" alt="" width="235" height="140"/></a>';
				$return_html.= '</div>';
			break;
			// end external link
			...
			case 'Image':
				$return_html.= '<div style="'.$current_float.'">';
				$return_html.= '<a title="'.$recent_portfolio->post_title.'" href="'.$image_url[0].'" class="img_frame" style="display:block;">';
				$return_html.= '<div class="overlay_detail" style="width:238px">
									<div>
									    <img src="'.get_stylesheet_directory_uri().'/images/icon_zoom.png" alt=""/><br/>
									    <strong>'.$recent_portfolio->post_title.'</strong>
			...
			case 'Youtube Video':
				$return_html.= '<div style="'.$current_float.'">';
				$return_html.= '<a title="'.$recent_portfolio->post_title.'" href="#video_'.$portfolio_video_id.'" class="lightbox_youtube" style="display:block;">';
				$return_html.= '<div class="overlay_detail" style="width:238px">
									<div>
				...
				$return_html.= '<div style="display:none;">
							    <div id="video_'.$portfolio_video_id.'" style="width:640px;height:385px">
							        ...
							        <object type="application/x-shockwave-flash" data="http://www.youtube.com/v/'.$portfolio_video_id.'" style="width:640px;height:385px" wmode="opaque">
			...
			case 'Vimeo Video':
				...
				$return_html.= '<div style="display:none;">
							    <div id="video_'.$portfolio_video_id.'" style="width:601px;height:338px">
							        ...
							        <object width="601" height="338" data="http://vimeo.com/moogaloop.swf?clip_id='.$portfolio_video_id.'&amp;server=vimeo.com&amp;show_title=0&amp;show_byline=0&amp;show_portrait=0&amp;color=ffffff&amp;fullscreen=1" type="application/x-shockwave-flash" wmode="opaque">
			  				    		<param name="allowfullscreen" value="true" />
			  				    		<param name="allowscriptaccess" value="always" />
...
function home_posts_func($atts, $content) {
	...
	$return_html = '<div class="post_header"><h3><a href="javascript:;">'.$title.'</a></h3></div>';
	$return_html.= '<div class="one_fourth">'.$content.'</div>';
	...
	foreach($recent_posts_arr as $key => $recent_portfolio)
	{
		if(($key+1)%3 == 0)
		{
			$column_class = 'one_fourth last';
		...
		$permalink = get_permalink($recent_portfolio);
		
		$return_html.= '<div class="'.$column_class.'"><a href="'.$permalink.'"><strong>'.$recent_portfolio->post_title.'</strong></a><br/>';
		$return_html.= '<div style="margin:5px 0 5px 0;font-size:11px"><img src="'.get_stylesheet_directory_uri().'/images/calendar.gif" class="middle" alt=""/>Posted on '.get_the_time('M d, Y').'</div>';
		$return_html.= '<br/><span>'.pp_substr(strip_tags(strip_shortcodes($recent_portfolio->post_content)), 170).'</span></div>';
	}
...
add_shortcode('home_posts', 'home_posts_func');
